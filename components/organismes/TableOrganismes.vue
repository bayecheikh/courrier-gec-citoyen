...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listorganismes : listorganismes"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listorganismes.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      ...
      headers: 'organismes/headerorganismes',
    ...
    methods: {
      ...
      editItem (item) {
        ...
        this.$router.push('/organismes/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "organismes"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)
