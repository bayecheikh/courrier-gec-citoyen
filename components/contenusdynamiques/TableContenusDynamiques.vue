...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listcontenusdynamiques : listcontenusdynamiques"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listcontenusdynamiques.length?false:true" 
    ...
    methods: {
      ...
      editItem (item) {
       ...
       deleteItem (item) {
        ...
        .then(async (response) => { 
            ...
            this.$store.dispatch('contenusdynamiques/deletecontenudynamique',this.activeItem.id)
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "contenusdynamiques"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)
