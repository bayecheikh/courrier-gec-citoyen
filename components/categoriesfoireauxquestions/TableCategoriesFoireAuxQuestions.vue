...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listcategoriesfoireauxquestions : listcategoriesfoireauxquestions"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listcategoriesfoireauxquestions.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listcategoriesfoireauxquestions: 'categoriesfoireauxquestions/listcategoriesfoireauxquestions',
      headers: 'categoriesfoireauxquestions/headercategoriesfoireauxquestions',
    ...
    methods: {
      ...
      editItem (item) {
        ...
        this.$router.push('/categoriesfoireauxquestions/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$gecApi.$delete('/categorie-faq/'+this.activeItem.id)
        .then(async (response) => { 
