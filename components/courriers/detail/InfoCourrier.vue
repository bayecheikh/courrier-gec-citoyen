<template>
  <v-card flat class="pl-5 pr-5 pt-5 pb-5">
    <v-card-text>
      ...
      <v-row class="border-grey mb-5 pl-5 pr-5 pb-5">
        <v-col md="12" sm="12" lg="12" class="row d-flex justify-start">
          <div class="col-md-6 col-sm-12 col-lg-6">
            <div class="d-flex text-label mb-5">
              Objet :
              <div class="text-green text-value">
                {{ this.detailCourrier && this.detailCourrier.subject }}
              </div>
            </div>
            <div class="d-flex text-label mb-5">
              Organisme :
              <div class="text-green text-value">
                {{
                  this.detailCourrier &&
                  this.detailCourrier.structure &&
                  this.detailCourrier.structure.description
                }}
              </div>
            </div>
            <div class="d-flex text-label mb-5">
              Code de suivi :
              <div class="text-green text-value">
                {{ this.detailCourrier && this.detailCourrier.idSuivi }}
              </div>
            </div>
          </div>
          <div class="col-md-6 col-sm-12 col-lg-6">
            <div class="d-flex text-label mb-5">
              ...
              <div class="text-green text-value">
                {{
                  $getDateFormat(
                    this.detailCourrier && this.detailCourrier.documentDate
                  )
                }}
              </div>
            </div>
            ...
            <div class="d-flex text-label mb-5">
              Statut :
              <div class="text-green text-value ml-3 mb-status">
                <v-chip
                  :color="
                    $getColore(
                      this.detailCourrier &&
                        this.detailCourrier.traitement_status_slug
                    )
                  "
                  outlined
                >
          ...
          <div
            class="col-md-12 col-sm-12 col-lg-12 border-grey"
            v-if="
              this.detailCourrier.responses &&
              this.detailCourrier.responses.length
            "
          >
            ...
            <div
              class="border-grey pl-5 pr-5 pt-5 mb-5"
              v-for="(reponse, index) in this.detailCourrier.responses"
              :key="index"
            >
              <div class="d-flex text-label mb-5">
                Date d'envoi :
                <div class="text-green text-value">
                  {{ $getDateFormat(reponse.send_date) }}
                </div>
              </div>
              <div class="d-flex text-label mb-5">
                Objet : <span v-html="reponse.object"></span>
              </div>
              ...
              <v-row
                class=""
                v-if="reponse.document && reponse.document.attachments"
              >
                <v-col
                  lg="3"
                  md="3"
                  sm="12"
                  v-for="(file, index) in reponse.document.attachments"
                  :key="index"
                  class=""
                >
                  <template>
                    <div class="text-center">
                      ...
                      <div class="d-flex justify-start flex-column">
                        <img
                          class="file"
                          :ref="'file' + file.id"
                          @click="getDocument(file.id)"
                          src="@/static/images/icons/file.png"
                          width="50"
                        />
                        <span class="d-flex justify-star"
                          >Pièce-jointe ({{ index + 1 }})</span
                        >
                        ...
                        <v-col md="12" lg="12" sm="12">
                          ...
                          <div
                            v-if="
                              !progress &&
                              document_link &&
                              document_link.originalFormat == 'pdf'
                            "
                          >
                            <embed
                              height="800"
                              :src="
                                'data:' +
                                document_link.mimeType +
                                ';base64,' +
                                document_link.encodedDocument +
                                '#toolbar=0'
                              "
                              class="embeded-courrier col-12"
                            />
                          </div>
                          <div
                            v-if="
                              !progress &&
                              document_link &&
                              document_link.originalFormat == 'docx'
                            "
                          >
                            <embed
                              height="800"
      ...
      <v-row
        class="border-grey mb-5"
        v-if="!this.detailCourrier.responses.length"
      >
        <v-col
          md="12"
          sm="12"
        ...
        >
          <div class="col-md-12 col-sm-12 col-lg-12">
            ...
            <div
              class="card"
              v-if="
                this.detailCourrier.pieces_jointes &&
                this.detailCourrier.pieces_jointes.length
              "
            >
              <v-row class="">
                <v-col
                ...
                >
                  <a
                    v-if="file.title"
                    class="custom-link d-flex align-items-start flex-column"
                    target="_blank"
                    :href="'data:application/pdf;base64,' + file.encodedFile"
                  >
            ...
            >
              <embed
                ...
                :src="
                  ...
                  this.detailCourrier.encodedFile +
                  '#toolbar=0'
                "
                class="embeded-courrier col-12"
              />
...
export default {
  ...
  methods: {
    ...
    getDocument(id) {
      ...
      this.$gecApi
        .$get("/attachments/" + id + "/" + idStructure)
        .then(async (response) => {
