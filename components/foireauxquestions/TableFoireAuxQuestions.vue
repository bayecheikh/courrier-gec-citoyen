...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listfoireauxquestions : listfoireauxquestions"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  ...
  export default {
    ...
    methods: {
      ...
      editItem (item) {
        ...
        this.$router.push('/foireauxquestions/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$gecApi.$delete('/faqs/'+this.activeItem.id)
        .then(async (response) => { 
            console.log('Réponse delete ++++++: ', response)
            this.$store.dispatch('foireauxquestions/deletefoireauxquestions',this.activeItem.id)
